    ...
    >
      ...
      >
        <span
          className={cn(
            'font-medium',
            activeMenuItems?.includes(item.key) ? 'text-primary' : 'text-gray-800 '
          )}
        >
          {item.label}
        </span>
...
const AppMenu = ({menuItems}: AppMenuProps) => {
  ...
  const activeMenu = useCallback(() => {
    // Use an empty string as fallback if location is undefined
  ...
  return (
    <ul className='navbar-nav flex gap-x-8 items-center justify-center' ref={menuRef}>
      {(menuItems ?? []).map(item => {
        return (
