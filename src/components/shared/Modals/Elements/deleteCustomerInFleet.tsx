...
const DeleteModal = el => {
  ...
  const [DeleteCustomerInFleet] = useMutation(mutations.DeleteCustomerInFleet);
  const {data: session} = useSession();
  const router = useRouter();
  const {t} = useTranslation();
  const {hideModal} = ModalContextProvider();
  return (
    <Modal title={`${t('common:delete_customer')}`} extraSmall>
      <div className='flex flex-row-reverse flex-wrap sm:flex-nowrap gap-2 mt-2'>
        <Button
          ...
          onClick={async () => {
            try {
              setSubmitting(true);
              const response = await DeleteCustomerInFleet({
                variables: {
                  fleetId: el.fleetId,
              ...
              setSubmitting(false);
              if (response?.data?.deleteCustomerInFleet?.response?.status !== 200) {
                setError(response?.data?.deleteCustomerInFleet?.response?.message);
                return;
              }
              setError('');
              router.reload();
